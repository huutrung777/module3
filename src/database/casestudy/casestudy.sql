...
select dvdk.ten_dich_vu_di_kem, sum(hdct.so_luong) as tong_so_luong
from dich_vu_di_kem dvdk
join hop_dong_chi_tiet hdct on dvdk.ma_dich_vu_di_kem = hdct.ma_dich_vu_di_kem
...
join (
    ...
    having (ifnull(sum(hdct.so_luong * dvdk.gia), 0) + ifnull(sum(dv.chi_phi_the), 0)) > 10000000
) as kh_tong
on kh.ma_khach_hang = kh_tong.ma_khach_hang
